...
void print_iterations(const std::string& description, const std::vector< std::vector<int> >& vectors, const int n, double time) {
    ...
    for (int i = 0; i != n; i++)
    {
        for (int j = 0; j != 4; j++)
        {
            ...
            if (it != vector.end()) { strings[j] += "*"; }
            else { strings[j] += " "; }
        }
    }
    std::cout << description << std::endl;
    for (auto& s : strings) { std::cout << s << "\n"; }
    std::cout << "Tempo de execução: " << time << std::endl;
    std::cout << std::endl;
}
...
void schedule(T function, const std::string& description, const int n)
{
    ...
    print_iterations(description, vectors, n, final_time);
}
...
void scheduleDefault(std::vector< std::vector<int> >& vectors, int n) {
    ...
    {    
        #pragma omp for 
        for (int i = 0; i < n; i++) { vectors[omp_get_thread_num()].push_back(i); }
    }
}
